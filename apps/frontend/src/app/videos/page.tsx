...
export default function VideosPage() {
  ...
  return (
    <PublicLayout>
      <section className="relative overflow-hidden border-b border-emerald-900/40 bg-[radial-gradient(circle_at_top,_rgba(52,211,153,0.28),_transparent_38%),linear-gradient(135deg,_#052e2b_0%,_#064e3b_52%,_#022c22_100%)] py-24 text-white">
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 gap-6 md:grid-cols-2 xl:grid-cols-3">
              {filteredSeries.map((series) => {
                ...
                return (
                <Link key={series.key} href={`/videos/${series.slug}`} className="group overflow-hidden rounded-[2rem] border border-white/70 bg-white shadow-[0_20px_50px_-30px_rgba(15,23,42,0.25)]">
                  <div className="relative aspect-video overflow-hidden bg-slate-900">
                    {thumbnail ? (
                      <img src={thumbnail} alt={series.title} className="h-full w-full object-cover opacity-75 transition-transform duration-700 group-hover:scale-105 group-hover:opacity-90" />
                    ) : (
                  ...
                  <div className="p-6">
                    ...
                    <h2 className="text-2xl font-black uppercase tracking-tight text-slate-900">{series.title}</h2>
                    {series.eventDate && (
            ...
            <PublicEmptyState
              ...
              description="Coba gunakan kata kunci lain atau cek kembali setelah dokumentasi video diperbarui."
